...
powershell -NoProfile -Command "$p=Get-CimInstance Win32_Process -Filter \"Name='python.exe'\" | Where-Object { $_.CommandLine -like '*sqs_local_worker.py*' }; if($p){ Write-Host '[status] SQS worker process=running' } else { Write-Host '[status] SQS worker process=not running' }"
